...
module.exports = async () => {
  ...
  const ManageContract = await ethers.getContractFactory("ManageContract");
  ...
  const manageContract = await ManageContract.deploy(deployArgs.backend);
  //We use wait to receive the transaction (deployment) receipt, which contains contractAddress
  const contractAddress = (await manageContract.deployTransaction.wait())
    .contractAddress;

  console.log(`ManageContract deployed to: ${contractAddress}`);
